...
export FLASK_SECRET_KEY="${FLASK_SECRET_KEY:-change_me_in_production}"
...
log "INFO" "Starting news server with Web UI"
...
# 3. Start Flask Web UI in background
log "INFO" "Starting Web UI"
cd /opt/webui
su - calibre -c "cd /opt/webui && gunicorn -b 0.0.0.0:5000 app:app --daemon --access-logfile $LOG_DIR/webui_access.log --error-logfile $LOG_DIR/webui_error.log"

# 4. Start calibre-server as calibre user
